...
import { useAdmin, User } from "../_context/AdminContext";
import { Search, ShieldAlert, CheckCircle, Trash2, Loader2, RefreshCw } from "lucide-react";

export default function VendorDirectory() {
  const { vendors, loadVendors, updateUserStatus, deleteAccount, isLoading } = useAdmin();
  ...
  useEffect(() => {
    const delayDebounce = setTimeout(() => {
      // Pass empty string for KYC filter since we've removed it
      loadVendors(searchQuery, "", statusFilter);
    }, 400);

    return () => clearTimeout(delayDebounce);
  }, [searchQuery, statusFilter, loadVendors]);
  ...
  const handleToggleStatus = async (userId: string, currentStatus: boolean) => {
    try {
      ...
      const message = err instanceof Error ? err.message : "Failed to toggle vendor account status.";
      alert(message);
    }
  };

  const handleDeleteVendor = async (userId: string, vendorName: string) => {
    if (confirm(`Are you absolutely sure you want to delete vendor "${vendorName}"? This will cascade to purge all associated listings, bookings, and chats.`)) {
      try {
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <input
            type="text"
            placeholder="Search vendors by business name, email, phone..."
            value={searchQuery}
          ...
          <table className="w-full border-collapse text-left">
            <thead>
              <tr className="border-b border-colonial/10 bg-black/25">
                <th className="px-6 py-4.5 text-xs font-bold uppercase tracking-wider text-colonial/50">Business Profile</th>
                ...
                <th className="px-6 py-4.5 text-xs font-bold uppercase tracking-wider text-colonial/50">Active</th>
              ...
              {vendors.length > 0 ? (
                  ...
                  return (
                    <tr key={vendor._id} className="hover:bg-white/[0.02] transition-colors">
                      <td className="px-6 py-4.5 whitespace-nowrap">
                        ...
                        <div className="text-colonial/50 font-mono text-xs mt-0.5">{vendor.email}</div>
                      </td>
                      <td className="px-6 py-4.5 text-colonial/80 text-sm font-mono whitespace-nowrap">{vendor.phone || "N/A"}</td>
                      <td className="px-6 py-4.5 text-colonial/60 text-xs font-mono whitespace-nowrap">{dateStr}</td>
                      <td className="px-6 py-4.5 whitespace-nowrap">
                        <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[10px] font-extrabold uppercase border ${
                          vendor.isActive 
                            ? "bg-emerald-500/5 border-emerald-500/20 text-emerald-400" 
                            : "bg-red-500/5 border-red-500/20 text-red-400"
                        }`}>
                          {vendor.isActive ? "Yes" : "Suspended"}
                        </span>
                      </td>
                      <td className="px-6 py-4.5 text-right whitespace-nowrap">
                        <div className="flex items-center justify-end gap-2.5">
                          ...
                          >
                            {vendor.isActive ? <ShieldAlert className="h-4 w-4" /> : <CheckCircle className="h-4 w-4" />}
                          </button>
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-colonial/40 font-medium">
                    {isLoading ? "Fetching records..." : "No vendors found. Try adjusting filter or search criteria."}
                  </td>
                </tr>
              )}
            </tbody>
